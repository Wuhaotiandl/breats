...
def draw_coords_img(img, coords1, coord2,  value1, value2):
    """
        在img上描边， 不影响原图
        ---------------------
        Parameters:
    ...
    for coord in coords1:
        _img[int(coord[0]), int(coord[1])] = value1
    for coord in coord2:
        _img[int(coord[0]), int(coord[1])] = value2
    return _img

def draw_coords_img_public(img, coords1, coord2,  value1, value2, public_value):
    """
        在img上描边， 不影响原图
        ---------------------
        Parameters:
    ...
    for coord in coords1:
        if coord in coord2:
            _img[int(coord[0]), int(coord[1])] = public_value
        else:
            _img[int(coord[0]), int(coord[1])] = value1
    for coord in coord2:
    ...
    for i in range(mask.shape[0] -1):
        for j in range(mask.shape[1] -1):
            ...
            if i > 0 and j > 0 :
                 ...
                 if sums > value and  abs(mask[i, j]-value) < 0.5 and sums < 8*0.8:
                     coords.append([i, j])
    return coords
